...
function isBot(author: GraphqlAuthor | null | undefined, botLogins: ReadonlySet<string>): boolean {
  const login = author?.login ?? "";
  return author?.__typename === "Bot" || botLogins.has(login) || login.toLowerCase().includes("[bot]");
}
...
export class GitHubClient {
  constructor(
    ...
    /** Review bots that GitHub reports as regular users; defaults cover the common hosted reviewers. */
    private readonly botLogins: ReadonlySet<string> = new Set(DEFAULT_BOT_LOGINS),
  ) {}
  ...
  private async fetchAdditionalComments(
    ...
    while (after) {
      const response: { node?: { __typename?: string; comments?: CommentConnection } | null } = await this.graphql(
