...
export default function addressReviewCommentsExtension(pi: ExtensionAPI): void {
  ...
  const reviewConfig = createReviewConfigCache();
  ...
  const runReviewCommand = async (args: string, ctx: ExtensionCommandContext) => {
    ...
    try {
      ...
      const client = new GitHubClient(exec, repositoryRoot, config.botLogins);
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    if (activeWorkflow && !activeWorkflow.githubUsername) {
      try {
        const client = new GitHubClient(commandExecutor(pi), activeWorkflow.repoRoot, reviewConfig(ctx.cwd).botLogins);
